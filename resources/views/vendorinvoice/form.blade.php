...
@else
@php  $route = url($confirmvendorinvoice); @endphp
				...
				<div class="x_panel">
					...
					<form id="frm_single" method="post" autocomplete="off" action ="{{$route}}"   class="form-horizontal form-label-left">
                                    ...
                                    <select id="vendor_id" name="vendor_id" style="width:100%;"  class="form-control col-md-7 col-xs-12 select2_single vendor_id" @if(Auth::user()->usertype_id !=1){{'disabled'}} @endif >
                                        <option value=""></option>
                                           @foreach($vendors as $vendorVal)
                                               <option @if($vendor_id != '') {{ ($vendor_id == $vendorVal->id) ? 'selected' : '' }}  @endif  value="{{ $vendorVal->id}}" @if($action=='update' || $action == 'view') {{ $vendorVal->id==$parisishthab->vendor_id ? 'selected':''}} @endif >{{ $vendorVal->vendor_name}}</option>
							...
							<div class="form-group">
                                ...
                                <div class="col-md-6 col-sm-6 col-xs-12">
                                    <input  type="text" id="invoice_no" name="invoice_no"  class="form-control col-md-7 col-xs-12 invoice_no" value="{{ ($action=='update' || $action == 'view') ? $parisishthab->invoice_no : old('invoice_no') }}">
                                                ...
                                                @if($date[$i] > date('Y-m-d'))
                                                    <tr class="parishishtha_b">
                                                        ...
                                                        <td>
                                                           ...
                                                           <select  id="parking_exp[{{$i}}]" name="parking_exp[{{$i}}]"  class="form-control select2_single col-md-7 col-xs-12 parking_exp" style="width:100%;" disabled>
                                                                <option value=""></option>
							...
							<table class="table table-bordered">
                                ...
                                <tr>
                                    ...
                                    <td width="10%"><input type="text" class="form-control total_amount amountonly" name="total_amount" value="@if($action=='update' || $action=='view'){{ $parisishthab->total_amount }}@endif" id="total_amount" readonly ></td>
								</tr>
                                <tr>
                                    <th colspan="3" style="text-align:right;">Total Deduction</th>
                                    <td width="10%"><input type="text" class="form-control total_charge amountonly" name="total_charge" value="@if($action=='update' || $action=='view'){{ $parisishthab->total_charge }} @endif" id="total_charge" readonly ></td>
								</tr>
                                <tr>
                                    <th colspan="3" style="text-align:right;"><span style="font-size:20px;">Grand Total</span></th>
                                    <td width="10%"><input type="text" class="form-control grand_amount amountonly" name="grand_amount" value="@if($action=='update' || $action=='view'){{ $parisishthab->grand_amount }} @endif" id="grand_amount" readonly ></td>
								</tr>
							</table>
  ...
  function cloneauto(days)
  {
   ...
   // $("#parishishtha_b tbody").html($trclone);
    ...
    //  return false;
                    ...
                    if (data.status) {
                        ...
                        $.each(data.routes,function(i,v) {
                            @if(isset($parisishthab->route_id))
                                    ...
                                    {
                                        routhtml += '<option value="'+v.id+'" selected>'+v.from_depot+' - '+v.to_depot+'</option>';

                                    }
                                    else
                                    {
                                        routhtml += '<option value="'+v.id+'" >'+v.from_depot+' - '+v.to_depot+'</option>';
                                    }
                                @else
                                    routhtml += '<option value="'+v.id+'" >'+v.from_depot+' - '+v.to_depot+'</option>';
                                @endif
                        ...
                        var vehiclehtml = '<option value="" ></option>';
                        $.each(data.vehicle,function(i,v) {
                            @if(isset($parisishthab->vehicle_id_reff))
                                    if(v.id=="{{$parisishthab->vehicle_id_reff}}")
                                    {
                                        vehiclehtml += '<option value="'+v.id+'" selected >'+v.vehicle_no+'</option>';
                                    }
                                    else
                                    {
                                        vehiclehtml += '<option value="'+v.id+'" >'+v.vehicle_no+'</option>';
                                    }
                                @else
                                    vehiclehtml += '<option value="'+v.id+'" >'+v.vehicle_no+'</option>';
                                @endif
                        });
                        $('#vehicle_id_reff').html(vehiclehtml);
                    }
                }
            });
        });
        ...
        {
            var res = false;
            var vahicleId = $('body').find('#vehicle_id_reff').val();
            var fromDate = $('body').find('#from_date').val();
            var toDate = $('body').find('#to').val();
            if (vahicleId != '' && fromDate != '' && toDate != '') {
